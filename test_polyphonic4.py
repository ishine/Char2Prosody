...
from train_polyphonic4 import G2PTransformerMask, Mask_Softmax_Eval
...
def main(args):
    ...
    with open(args.emo_tts_test_file, encoding='utf-8', errors='ignore') as f:
            ...
            if label_idxs != []:
                ...
                for i in range(len(label_idxs)):
                    if label_idxs[i] != preds[i]:
                        ...
                        file_out.append(
                            [''.join(wrong_sen), str(int(poly_idxs[i])), wrong_sen[poly_idxs[i]],
                             idx2class[int(label_idxs[i])], idx2class[int(preds[i])], flag])
                        # print(file_out[-1])
        ...
        # with open('./filelists/wrongcase_nomask_emoTTS.txt', 'w', encoding='utf8') as fout:
        #     fout.write('\n'.join(file_out))
        column = ['句子内容', '字的位置', '字', '标注读音', '预测读音', '是否在g2pM集']
        test = pd.DataFrame(columns=column, data=file_out)
        test.to_csv('./filelists/wrongcase_nomask_emoTTS.csv', encoding='utf_8')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--saved_model_path", type=str, default="./save/no_mask/97.85_model.pt")
