...
def prepare_dataloaders(hparams):
    ...
    collate_fn = TextMelCollate(hparams.n_frames_per_step, hparams.n_pinyin_symbols)
...
def load_model(hparams, pretrain_path):
    model = Tacotron2(hparams, pretrain_path).cuda()
    if hparams.fp16_run:
...
def validate(model, criterion, valset, iteration, batch_size, n_gpus,
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            ...
            loss = criterion(y_pred, y, mask_padded)
            if distributed_run:
...
def train(output_directory, log_directory, pretrain_path, checkpoint_path, warm_start, n_gpus,
    ...
    model = load_model(hparams, pretrain_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--pretrain_path', type=str, default='./models/ch_bert_bmes_dev=93.97.pt',
                        required=False, help='pretrained beparse path')
    ...
    train(args.output_directory, args.log_directory, args.pretrain_path, args.checkpoint_path,
