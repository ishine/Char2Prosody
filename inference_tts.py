...
def inference(args):
    ...
    test_loader = DataLoader(test_set, num_workers=0, shuffle=shuffle, sampler=test_sampler, batch_size=hparams.batch_size, pin_memory=False, drop_last=True, collate_fn=test_collate_fn)
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_loader):
            ...
            for j in range(mel_outputs.size(0)):
                ...
                mel_decompress = taco_stft.spectral_de_normalize(mel_outputs_postnet[j])
                ...
                audio_path = os.path.join(args.out_filename, 'batch_{}_sentence_{}'.format(i, j))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-w', '--sentences', type=str, help='text to infer', default=['南国冬日里，寒风冻死人不偿命'])
    parser.add_argument('-t', '--text_file', type=str, help='text file to infer', default='./textToSynthesize.txt')
