...
from train_polyphonic import Mask_Softmax_Eval, get_dataloader
from models import G2PTransformerMask
...
def main(args):
    ...
    for batch in tqdm(test_dataloader, total=len(test_dataloader)):
        ...
        for i in range(len(preds)):
            if labels[i] != preds[i]:
                ...
                file_out.append(''.join(wrong_sen) + '|' + wrong_sen[poly_ids[i]] + '|' + idx2class[int(labels[i])] + '|' + idx2class[int(preds[i])])
                print(file_out[-1])
    ...
    with open('./filelists/wrongcase_mask_g2pM.txt', 'w', encoding='utf8') as fout:
        fout.write('\n'.join(file_out))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--saved_model_path", type=str, default="'./save/poly_only_syntax/97.90_model.pt'")
