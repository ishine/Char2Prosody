...
def get_hop_size(hparams):
	...
	if hop_length is None:
		...
		hop_length = int(hparams.frame_shift_ms / 1000 * hparams.sampling_rate)
	return hop_length
...
def _lws_processor(hparams):
	import lws
	return lws.lws(hparams.filter_length, get_hop_size(hparams), fftsize=hparams.win_length, mode="speech")
...
def _griffin_lim_tensorflow(S, hparams):
	...
	with tf.variable_scope('griffinlim'):
		...
		y = tf.contrib.signal.inverse_stft(S_complex, hparams.win_length, get_hop_size(hparams), hparams.filter_length)
		for i in range(hparams.griffin_lim_iters):
			est = tf.contrib.signal.stft(y, hparams.win_length, get_hop_size(hparams), hparams.filter_length)
			angles = est / tf.cast(tf.maximum(1e-8, tf.abs(est)), tf.complex64)
			y = tf.contrib.signal.inverse_stft(S_complex * angles, hparams.win_length, get_hop_size(hparams), hparams.filter_length)
...
def _stft(y, hparams):
	if hparams.use_lws:
		...
		return librosa.stft(y=y, n_fft=hparams.filter_length, hop_length=get_hop_size(hparams), win_length=hparams.win_length, pad_mode='constant')
