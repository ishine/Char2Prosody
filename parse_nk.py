...
class FeatureDropoutFunction(torch.autograd.function.InplaceFunction):
    ...
    def forward(cls, ctx, input, batch_idxs, p=0.5, train=False, inplace=False):
        ...
        # print('CHECK cls:', cls)
        # print('CHECK ctx:', ctx)
        # print('CHECK input:', input.shape)
        # print('CHECK batch_idxs:', batch_idxs)
        # print('CHECK p:', p)
        # print('CHECK train:', train)
        # print('CHECK inplace:', inplace)
        ...
        if ctx.p > 0 and ctx.train:
            ...
            # print('CHECK noise:', ctx.noise.shape)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, inp, batch_idxs, qk_inp=None):
        ...
        # print('CHECK outputs in MultiHeadAttention:', outputs.shape)

        outputs = self.residual_dropout(outputs, batch_idxs)

        return self.layer_norm(outputs + residual), attns_padded

    def select_forward(self, inp):
        residual = inp

        # While still using a packed representation, project to obtain the
        # query/key/value for each head
        q_s, k_s, v_s = self.split_qkv_packed(inp, qk_inp=None)

        # Switch to padded representation, perform attention, then switch back
        # q_padded, k_padded, v_padded, attn_mask, output_mask = self.pad_and_rearrange(q_s, k_s, v_s, batch_idxs)

        outputs, attns_padded = self.attention(q_s, k_s, v_s,)
        # outputs = outputs_padded[output_mask]
        outputs = self.combine_v(outputs)
...
class Encoder(nn.Module):
    ...
    def forward(self, xs, batch_idxs, extra_content_annotations=None):
        ...
        return res, batch_idxs, embedding_output
...
class NKChartParser(nn.Module):
    ...
    def __init__(
        ...
        if hparams.use_bert or hparams.use_bert_only:
            ...
            assert self.emb_types, "Need at least one of: use_words, use_bert, use_bert_only"
    ...
    def parse_batch(self, sentences, golds=None, return_label_scores_charts=False):
        ...
        # print('CHECK is_train IN tree_encoder parse_batch:', is_train)
        self.train(is_train)
        ...
        # print('CHECK sentences IN parse_batch:', len(sentences))
        # print('CHECK sentences in parse_batch:', sentences)

        packed_len = sum([(len(sentence) + 2) for sentence in sentences])
        # print('CHECK packed_len in parse_batch:', packed_len)
        ...
        for snum, sentence in enumerate(sentences):
            ...
            for word in [START] + sentence + [STOP]:
        ...
        if self.bert is not None:
            ...
            for snum, sentence in enumerate(sentences):
                ...
                # print('CHECK sentence in parse_batch of bert:', sentence)
            ...
            if self.encoder is not None:
                ...
                # print('CHECK extra_content_annotations:', extra_content_annotations)
                # print('CHECK extra_content_annotations SHAPE:', extra_content_annotations.shape)

        # print('CHECK Encoder input emb_idxs:', emb_idxs)
        # print('CHECK Encoder input batch_idxs:', batch_idxs)
        # print('CHECK Encoder input annotations:', extra_content_annotations)
        # print('CHECK Encoder input annotations shape:', extra_content_annotations.shape)

        if self.encoder is not None:
            annotations, _, embedding_output = self.encoder(emb_idxs, batch_idxs, extra_content_annotations=extra_content_annotations)

            # print('CHECK Encoder output embedding_output:', embedding_output)
            # print('CHECK Encoder output embedding_output shape:', embedding_output.shape)
            ...
            if self.partitioned:
                ...
                # print('CHECK Encoder output catted annotations:', annotations)
                # print('CHECK Encoder output catted annotations shape:', annotations.shape)
            ...
            # print('CHECK Encoder output fencepost_annotations:', fencepost_annotations)
            # print('CHECK Encoder output fencepost_annotations shape:', fencepost_annotations.shape)
        ...
        if return_label_scores_charts:
            ...
            for i, (start, end) in enumerate(zip(fp_startpoints, fp_endpoints)):
                original_features = fencepost_annotations[start:end,:]
                sentence_length = original_features.size(0)
                original_features = original_features[1:-1, :]
                original_features = to_gpu(original_features).float()
                original_features_padder = torch.zeros([len_padded - sentence_length, 1024])
                original_features_padder = to_gpu(original_features_padder).float()
                original_features_padded = torch.cat([original_features, original_features_padder], 0)
                hidden_features.append(original_features_padded)
            hidden_features_reshape = torch.cat(hidden_features, 0)
            hidden_features_reshape = torch.reshape(hidden_features_reshape,  [mb_size, -1, 1024])


            embedding_outputs = []
            for i, (start, end) in enumerate(zip(fp_startpoints, fp_endpoints)):
                original_embedding_features = embedding_output[start:end,:]
                sentence_length = original_embedding_features.size(0)
                original_embedding_features = original_embedding_features[1:-1, :]
                original_embedding_features = to_gpu(original_embedding_features).float()
                original_embedding_features_padder = torch.zeros([len_padded - sentence_length, 1024])
                original_embedding_features_padder = to_gpu(original_embedding_features_padder).float()
                original_embedding_features_padded = torch.cat([original_embedding_features, original_embedding_features_padder], 0)
                embedding_outputs.append(original_embedding_features_padded)
            embedding_outputs_reshape = torch.cat(embedding_outputs, 0)
            embedding_outputs_reshape = torch.reshape(embedding_outputs_reshape,  [mb_size, -1, 1024])
            ...
            return hidden_features_reshape, charts, embedding_outputs_reshape
        ...
        with torch.no_grad():
            for i, (start, end) in enumerate(zip(fp_startpoints, fp_endpoints)):
                # print('CHECK sentence:', sentences[i])
        ...
        # print('CHECK cells_label:', cells_label)
        # print('CHECK cells_label shape:', cells_label.shape)
        ...
        # print('CHECK cells_label_scores:', cells_label_scores)
        # print('CHECK cells_label_scores shape:', cells_label_scores.shape)

        cells_scores = torch.gather(cells_label_scores, 1, cells_label[:, None])

        # print('CHECK cells_scores:', cells_scores)
        # print('CHECK cells_scores shape:', cells_scores.shape)
    ...
    def parse_from_annotations(self, fencepost_annotations_start, fencepost_annotations_end, sentence, gold=None):
        ...
        # print('CHECK label_scores_chart IN parse_from_annotations:', label_scores_chart)
        # print('CHECK label_scores_chart IN parse_from_annotations shape:', label_scores_chart.shape)
    ...
    def decode_from_chart(self, sentence, chart_np, gold=None):
        ...
        def make_tree():
            ...
            if (i + 1) >= j:
                word = sentence[i]
                tree = trees.LeafParseNode(int(i), 'NS', word)
                if label:
